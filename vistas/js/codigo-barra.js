$(document).ready(function () {
            ...
            var fila = `
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button href="#" class="me- btn btn-primary btnGenerarCodigoBarra" idProducto="${producto.id_producto}" nombreProducto="${producto.nombre_producto}" codigoProducto="${producto.codigo_producto}" data-bs-toggle="modal" data-bs-target="#modalEditarProducto">
    ...
    $("#tabla_productos_codigo_barra").on("click", ".btnGenerarCodigoBarra", function(e){
    ...
    $("#btn_generar_codigo_producto").click(function(e) {
        
        e.preventDefault();

        let codigo_producto = $("#show_data_codigo_producto").val().trim(); // Cambiado a val() si es un input
        let cantidad = parseInt($("#cantidad_codigo_barra").val(), 10);
        let altura = parseInt($("#altura_codigo_barra").val(), 10);
        let ancho = parseInt($("#ancho_codigo_barra").val(), 10);

        // Limpiar lista anterior
        $("#lista_codigo_barra").empty();

        // Generar y añadir códigos de barras
        for (let i = 0; i < cantidad; i++) {
            let svg = document.createElement("svg");
            $(svg).css({height: altura + 'px', width: ancho + 'px'}); // Asegura el tamaño adecuado
            $("#lista_codigo_barra").append(svg);

            JsBarcode(svg, codigo_producto, {
                format: "CODE128",
                lineColor: "#000",
                width: ancho,
                height: altura,
