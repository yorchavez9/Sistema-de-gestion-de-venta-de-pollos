...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->setXY(60, 15);
        $this->Cell(100, 8, utf8_decode('Reporte de clientes'), 0, 1, 'C', 0);
...
$anchoN = 10;
$anchoNombre = 45;
$anchoDocumento = 25; // Reducido el ancho del documento
$anchoDireccion = 40;
$anchoTelefono = 25;
$anchoCorreo = 50;

$pdf->Cell($anchoN, 8, 'N', 'B', 0, 'L', 0);
$pdf->Cell($anchoNombre, 8, utf8_decode('Nombre'), 'B', 0, 'L', 0);
$pdf->Cell($anchoDocumento, 8, utf8_decode('Documento'), 'B', 0, 'L', 0);
$pdf->Cell($anchoDireccion, 8, utf8_decode('Dirección'), 'B', 0, 'L', 0);
$pdf->Cell($anchoTelefono, 8, utf8_decode('Teléfono'), 'B', 0, 'L', 0);
$pdf->Cell($anchoCorreo, 8, 'Correo', 'B', 1, 'L', 0);
...
foreach ($proveedores as $proveedor) {
    $pdf->Ln(0.6);
    $pdf->setX(10); // Ajuste para los márgenes
    $pdf->Cell($anchoN, 8, $contador, 'B', 0, 'L', 1); // Aquí debería ir el número de registro, adaptar según el contenido
    $pdf->Cell($anchoNombre, 8, utf8_decode($proveedor['razon_social']), 'B', 0, 'L', 1);
    $pdf->Cell($anchoDocumento, 8, utf8_decode($proveedor['numero_documento']), 'B', 0, 'L', 1);
    $pdf->Cell($anchoDireccion, 8, utf8_decode($proveedor['direccion']), 'B', 0, 'L', 1);
    $pdf->Cell($anchoTelefono, 8, utf8_decode($proveedor['telefono']), 'B', 0, 'L', 1);
    $pdf->Cell($anchoCorreo, 8, utf8_decode($proveedor['email']), 'B', 1, 'L', 1);
